...
CREATE TABLE members (
    ...
    create_at DATETIME NOT NULL DEFAULT current_timestamp,								# 계정 생성 날짜
    update_at DATETIME NOT NULL DEFAULT current_timestamp ON UPDATE current_timestamp,	# 계정 정보 수정 날짜
    sanctions_count INT NOT NULL DEFAULT 0,												# 제재 당한 횟수(최대 3회)
...
CREATE TABLE answers (
	...
	parent_id BIGINT NULL,																	# ✅ 부모 답변 ID(대댓글용)[FK] - NULL이면 최상위 댓글
...
-- parent_id는 대댓글용 부모 댓글 지정을 위한 컬럼으로 answers 테이블의 id를 self 참조한다.
-- NULL이면 최상위 댓글, 값이 있으면 대댓글. 부모 댓글 삭제 시 해당 댓글도 같이 삭제됨(CASCADE)
